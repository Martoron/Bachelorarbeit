# =============================================================================
# DATA ANALYSIS AND VISUALIZATION FOR DOKKAN BATTLE SIMULATION RESULTS
# =============================================================================
...
def parse_number(s):
    """
    Parse a number from string, handling various formats and commas.
    
    Args:
        s (str): String containing a number
...
def extract_metrics(file_path, condition_name, mode="team"):
    """
    Extract metrics from simulation CSV files.
    
    Args:
        file_path (str): Path to CSV file
        condition_name (str): Name of the condition being processed
        mode (str, optional): "team" for team metrics, "testunit" for specific unit metrics
    ...
    # Set unit filter for testunit mode
    unit_filter = None
    if mode == "testunit":
    ...
    for line in lines:
        ...
        # Accumulate damage based on mode
        if mode == "team":
...
def bootstrap_ci(data, n_bootstrap=10000, ci=95):
    ...
    Args:
        ...
        ci (int, optional): Confidence interval percentage. Defaults to 95
...
def bootstrap_ratio(numerators, denominators, n_bootstrap=10000, ci=95):
    """
    Calculate bootstrap confidence intervals for ratios.
    
    Args:
        numerators (array-like): Numerator values
        denominators (array-like): Denominator values
        n_bootstrap (int, optional): Number of bootstrap samples. Defaults to 10000
        ci (int, optional): Confidence interval percentage. Defaults to 95
...
def create_summary(mode, out_dir):
    """
    Create summary statistics with bootstrap confidence intervals.
    
    Args:
        mode (str): "team" or "testunit" mode
        out_dir (str): Output directory for results
        
    Returns:
        DataFrame: Summary statistics with confidence intervals
    """
    all_rows = []
...
def plot_with_highlight(df, metric, ci_lower, ci_upper, ylabel, title, filename, out_dir, legend_pos="default"):
    """
    Create bar plots with confidence intervals and condition highlighting.
    
    Args:
        df (DataFrame): Data to plot
        metric (str): Metric column to plot
        ci_lower (str): Lower CI column name
        ci_upper (str): Upper CI column name
        ylabel (str): Y-axis label
        title (str): Plot title
        filename (str): Output filename
        out_dir (str): Output directory
        legend_pos (str, optional): Legend position. Defaults to "default"
    """
...
def plot_special_actions(df, out_dir, mode="team"):
    """
    Plot special actions (heals, revives) comparison.
    
    Args:
        df (DataFrame): Special actions data
        out_dir (str): Output directory
        mode (str, optional): "team" or "testunit". Defaults to "team"
    """
    plt.figure(figsize=(8,5))
...
# =============================================================================
# BIS (BATTLE IMPACT SCORE) CALCULATION
# =============================================================================

def calculate_bis_adjusted(df_team, df_testunit):
    """
    Calculate adjusted Battle Impact Score (BIS) comparing LLM vs power conditions.
    
    Args:
        df_team (DataFrame): Team-level metrics
        df_testunit (DataFrame): Test unit-level metrics
        
    Returns:
        DataFrame: BIS values for each condition pair
    """
    # Calculate baseline from baseline conditions
    ...
    for power_cond, llm_cond, label in pairs:
        ...
        for metric in ["WinRate", "DmgReceived", "DmgDealt"]:
            ...
            # Adjust BIS based on test unit performance
...
# Create summaries for team and test unit levels
...
# Create visualizations for both team and test unit levels
for mode, df, out_dir in [("team", df_team, team_dir), ("testunit", df_testunit, testunit_dir)]:
    plot_with_highlight(df, "WinRate", "WinRate_CI_L", "WinRate_CI_U", "Win Rate", 
                       f"WinRate_{mode}", f"WinRate_{mode}.png", out_dir, legend_pos="lower left")
    plot_with_highlight(df, "DmgDealt", "DmgDealt_CI_L", "DmgDealt_CI_U", "Avg Damage Dealt / Turn", 
                       f"DmgDealt_{mode}", f"DmgDealt_{mode}.png", out_dir)
    plot_with_highlight(df, "DmgReceived", "DmgReceived_CI_L", "DmgReceived_CI_U", "Avg Damage Received / Turn", 
                       f"DmgReceived_{mode}", f"DmgReceived_{mode}.png", out_dir)

# Plot special actions
plot_special_actions(df_special_actions_team, team_dir, mode="team")


# =============================================================================
# BIS VISUALIZATION FUNCTIONS
# =============================================================================

def plot_bis_adjusted(df_bis, out_dir):
    """
    Plot adjusted BIS scores for different metrics.
    
    Args:
        df_bis (DataFrame): BIS scores data
        out_dir (str): Output directory
    """
    ...
    for metric in metrics:
        plt.figure(figsize=(8,5))
        # Color coding: orange for advantage conditions, blue for normal conditions
        ...
        plt.ylim(0, 1.05)  # BIS is normalized to 0-1
        plt.title(labels[metric], fontsize=22)
        plt.xticks(rotation=45, ha='right', fontsize=16)
        plt.yticks(fontsize=16)
        plt.axhline(0.3, color="green", linestyle="--", label="Successful (≤0.3)")
        plt.axhline(0.7, color="red", linestyle="--", label="Insufficient (>0.7)")
        plt.tight_layout()
...
def save_bis_legend(out_dir):
    """
    Save BIS legend as a separate image for layout purposes.
...
def plot_actions_vs_winrate(df_combined_metrics, out_dir, mode="team"):
    """
    Plot actions vs win rate comparison.
    
    Args:
        df_combined_metrics (DataFrame): Combined metrics data
        out_dir (str): Output directory
        mode (str, optional): "team" or "testunit". Defaults to "team"
    """
    ...
    if "HealsUsed" in df_combined_metrics.columns and "RevivesUsed" in df_combined_metrics.columns:
        ...
        # Separate bars for heals and revives
...
def plot_action_winrate_correlation(df, out_dir, mode="team"):
    """
    Plot correlation between actions and win rate.
    
    Args:
        df (DataFrame): Data for correlation analysis
        out_dir (str): Output directory
        mode (str, optional): "team" or "testunit". Defaults to "team"
    """
    ...
    if "HealsUsed" in df.columns and "RevivesUsed" in df.columns:
        ...
        raise ValueError("No valid metric column found.")
...
# Plot BIS scores and legend
plot_bis_adjusted(df_bis, team_dir)
save_bis_legend(team_dir)

print("Complete: Team and test unit evaluations including adjusted BIS created.")